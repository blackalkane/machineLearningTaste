...
function logRegL0(X,y,lambda)
	...
	while minScore != oldScore
		...
		for j in setdiff(1:d,S)
			...
			Sj = [S;j]
			Xs = X[:,Sj]
			wj = zeros(length(Sj),1)
			wj = findMin(funObj,wj,verbose=false)
			(fj,~) = funObj(wj)
			scorej = fj + lambda*length(Sj)
			if scorej < minScore
				minScore = scorej
				minS = Sj
			end
...
function softmaxFunObj(w,X,y)
	...
	for i in 1:n
		f_temp = 0
		for c in 1:k
			f_temp += exp(sum(W[c,:].*X[i,:]))
		end
		f += log(f_temp) - sum(W[y[i],:].*X[i,:])
	end
	# get g
	g = zeros(k,d)
	for c in 1:k
		for j in 1:d
			for i in 1:n
				...
				p_total = 0
				for c_temp in 1:k
					p_total += exp(sum(W[c_temp,:].*X[i,:]))
				end
				p_final = exp(sum(W[c,:].*X[i,:]))/p_total
				# check I(y_i == c)
				if (y[i] == c)
					p_final -= 1
				end
